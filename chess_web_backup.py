...
<head>
    ...
    <style>
        ...
        * {
        ...
        .chess-board { 
            display: grid;
            grid-template-columns: repeat(8, 68px);  /* Optimized size */
            grid-template-rows: repeat(8, 68px);
            width: 544px;  /* Updated to match new square size */
            height: 544px;
            border: 10px solid #654321;  /* Refined border */
            border-radius: 20px;  /* Enhanced corners */
            box-shadow: 
                0 30px 60px rgba(0,0,0,0.4),  /* Enhanced shadow */
                inset 0 0 40px rgba(139,69,19,0.3),
                0 0 0 6px #8b4513,  /* Border ring */
                0 0 0 12px rgba(139,69,19,0.2),
                0 0 100px rgba(101,67,33,0.3);  /* Ambient glow */
            background: 
                radial-gradient(circle at 25% 25%, rgba(255,255,255,0.1) 0%, transparent 50%),
                linear-gradient(135deg, #f0d9b5 0%, #ede0c8 100%);
            position: relative;
            gap: 0;
            transform: perspective(1500px) rotateX(2deg) rotateY(1deg);  /* Enhanced 3D */
            /* Performance optimizations */
            will-change: transform, box-shadow;
        ...
        .chess-square {
            width: 68px;  /* Updated to match board */
            height: 68px;
            display: flex;
            align-items: center;
            justify-content: center;
            font-size: 54px;  /* Enhanced piece size */
            font-family: 'Segoe UI Symbol', 'Apple Color Emoji', 'Noto Color Emoji', monospace;
            cursor: pointer;
            transition: all 0.25s cubic-bezier(0.25, 0.46, 0.45, 0.94);  /* Optimized timing */
            position: relative;
            user-select: none;
            box-sizing: border-box;
        ...
        .chess-square:hover {
            background: 
                radial-gradient(circle at 50% 50%, rgba(255,235,59,0.9) 0%, rgba(255,193,7,0.7) 100%),
                linear-gradient(135deg, #fff176 0%, #ffeb3b 50%, #ffc107 100%) !important;
            transform: scale(1.03) translateY(-2px);  /* Optimized for performance */
            z-index: 10;
            box-shadow: 
        ...
        @keyframes moveComplete {
            0% { 
                ...
                box-shadow: 0 0 35px rgba(76,175,80,0.9), inset 0 0 20px rgba(76,175,80,0.3);
                filter: brightness(1.3);
            }
            50% { 
                transform: scale(1.08) translateZ(0);
                box-shadow: 0 0 25px rgba(76,175,80,0.7), inset 0 0 15px rgba(76,175,80,0.2);
        ...
        .chess-square.move-animation {
            animation: moveComplete 0.9s cubic-bezier(0.25, 0.46, 0.45, 0.94);
        }
        ...
        /* Enhanced piece placement animation with GPU acceleration */
        @keyframes piecePlacement {
            0% { 
                transform: scale(0.3) translateY(-30px) rotateY(180deg) translateZ(0);
                opacity: 0;
                filter: blur(2px);
            }
            30% {
                transform: scale(0.8) translateY(-10px) rotateY(120deg) translateZ(0);
                opacity: 0.6;
                filter: blur(1px);
            }
            70% {
                transform: scale(1.15) translateY(-2px) rotateY(45deg) translateZ(0);
                opacity: 0.9;
                filter: blur(0);
            }
            100% { 
                transform: scale(1) translateY(0) rotateY(0deg) translateZ(0);
                opacity: 1;
                filter: blur(0);
            }
        }
        ...
        .chess-piece.placed {
            animation: piecePlacement 0.7s cubic-bezier(0.68, -0.55, 0.265, 1.55);
        }
        
        /* Enhanced piece capture animation */
        @keyframes pieceCapture {
            ...
            75% {
                transform: scale(0.3) rotate(45deg) translateY(-20px) translateZ(0);
                opacity: 0.2;
                filter: brightness(0.5);
            }
            100% { 
                transform: scale(0) rotate(90deg) translateY(-40px) translateZ(0);
                opacity: 0;
                filter: brightness(0);
            }
        }
        ...
        .chess-piece.captured {
            animation: pieceCapture 0.5s cubic-bezier(0.55, 0.085, 0.68, 0.53) forwards;
        }
        ...
        @keyframes boardLoader {
            0% { opacity: 0; transform: scale(0.8) rotateY(-10deg) translateZ(0); }
            100% { opacity: 1; transform: scale(1) rotateY(0deg) translateZ(0); }
        }
        
        .chess-board {
            animation: boardLoader 0.8s cubic-bezier(0.25, 0.46, 0.45, 0.94);
        }
...
<body>
    ...
    <!-- Game Over Modal -->
        ...
        function getOptimalPollingFrequency() {
            ...
            if (!isTabActive) return 5000; // Slow when tab inactive
            if (timeSinceActivity < 30000) return 1000; // Fast for first 30 seconds
            if (timeSinceActivity < 120000) return 2000; // Medium for 2 minutes
            return 3000; // Slower after 2 minutes of inactivity
        }
        let lastMoveTime = 0;
    ...
    <div id="checkmateModal" class="checkmate-modal">
        <div class="checkmate-content">
            ...
            <div class="checkmate-buttons">
                <button class="checkmate-btn primary" onclick="location.reload()">🔄 New Game</button>
                <button class="checkmate-btn secondary" onclick="closeCheckmateModal()">📊 Review Game</button>
            </div>
        </div>
    </div>
...
def add_chess_routes(app):
    """Add chess game routes to Flask app"""
                if (!playerNameEl) {
                    ...
                    alert('Error: Player name input not found');
                    return;
                }
                ...
                if (!playerName) {
                    ...
                    if (typeof showNotification === 'function') {
                        showNotification('Name Required', 'Please enter your name!', 'error');
                    } else {
            ...
            try {
                ...
                if (data.success) {
                    ...
                    showNotification('Creation Failed', 'Failed to create game: ' + data.error, 'error');
                }
            } catch (error) {
                console.error('Create game error:', error);
                if (typeof showNotification === 'function') {
                    showNotification('Connection Error', 'Error creating game: ' + error.message, 'error');
                } else {
                    alert('Error creating game: ' + error.message);
                }
            } finally {
                ...
                const createBtn = document.getElementById('createGameBtn');
                if (createBtn) {
                    createBtn.textContent = originalText || 'Create New Game';
        ...
        // Join existing game with enhanced error handling
        window.joinGame = async function() {
            ...
            if (!gameIdToJoin) {
                showNotification('Game ID Required', 'Please enter a Game ID!', 'error');
                return;
            }
            
            // Show loading state
            const joinBtn = document.getElementById('joinGameBtn') || event.target;
            const originalText = joinBtn.textContent;
            joinBtn.textContent = 'Joining...';
            joinBtn.disabled = true;
            
            try {
                console.log('Sending join game request for ID:', gameIdToJoin);
        ...
        // Start polling for game updates with optimizations
        function startGamePolling() {
            if (gamePollingInterval) clearInterval(gamePollingInterval);
            
            // Use adaptive polling frequency
            const pollFrequency = getOptimalPollingFrequency();
            
            gamePollingInterval = setInterval(async () => {
                try {
                    // Skip polling if tab is hidden and game is inactive
                    if (!isTabActive && (Date.now() - lastActivityTime) > 300000) {
                        return; // Skip polling after 5 minutes of inactivity
                    }
                    ...
                    if (data.success) {
                        ...
                        // Update player info efficiently
                    ...
                    pollingFrequency = Math.min(pollingFrequency * 1.5, 10000);
                }
            }, pollFrequency);
            ...
            setTimeout(() => {
                if (gamePollingInterval) {
                    const newFrequency = getOptimalPollingFrequency();
                    if (newFrequency !== pollFrequency) {
                        startGamePolling(); // Restart with new frequency
                    }
                }
            }, pollFrequency);
        }
        ...
        // Optimized board update with minimal DOM manipulation
        function updateBoardWithAnimation(boardString) {
            const oldBoard = boardData ? JSON.parse(JSON.stringify(boardData)) : null;
        ...
        // Efficient square animation with GPU acceleration
        function animateSquareChange(row, col) {
            const square = document.querySelector(`[data-row="${row}"][data-col="${col}"]`);
            if (square) {
                square.style.willChange = 'transform, box-shadow';
        ...
        // Optimized last move highlight
        function highlightLastMove() {
            // Use efficient class removal
            const previousHighlights = document.querySelectorAll('.last-move');
            previousHighlights.forEach(sq => sq.classList.remove('last-move'));
            
            // Add optimized highlight with requestAnimationFrame
            requestAnimationFrame(() => {
                const board = document.getElementById('chessBoard');
                if (board) {
                    board.style.willChange = 'box-shadow';
                    board.style.filter = 'drop-shadow(0 0 15px rgba(76,175,80,0.5))';
        ...
        // Optimized board rendering with virtual DOM concepts
        function renderBoard() {
            ...
            for (let row = 0; row < 8; row++) {
                for (let col = 0; col < 8; col++) {
                    const key = `${row}-${col}`;
                    let square = existingSquares[key];
                    
                    // Reuse existing square if possible, otherwise create new
                    if (!square) {
                    ...
                    // Update piece content efficiently
                    ...
                    if (piece !== ' ') {
                        ...
                        pieceElement.className = `chess-piece ${isWhitePiece ? 'white' : 'black'}`;
                        if (pieceElement.textContent !== pieceSymbol) {
                            pieceElement.textContent = pieceSymbol;
                        }
                        
                        // Enhanced piece styling with GPU acceleration
                        pieceElement.style.textShadow = isWhitePiece 
                            ? '0 2px 4px rgba(0,0,0,0.8), 0 0 8px rgba(255,255,255,0.3)'
                            : '0 2px 4px rgba(255,255,255,0.6), 0 0 8px rgba(0,0,0,0.5)';
                        pieceElement.style.willChange = 'transform, filter';
        ...
        // Enhanced square click handler with debouncing and optimizations
        let clickTimeout = null;
        let isProcessingMove = false;
        
        function handleSquareClick(event) {
            // Prevent rapid clicking and ensure game is ready
            if (isProcessingMove || !gameId || currentPlayer !== playerColor) {
                if (currentPlayer !== playerColor) {
                    // Throttle turn notifications
                    const currentTime = Date.now();
                    if (currentTime - lastNotificationTime > 2000) {
            ...
            clickTimeout = setTimeout(() => {
                processSquareClick(event);
            }, 50);
        }
        
        async function processSquareClick(event) {
            ...
            try {
                if (selectedSquare) {
                    ...
                    const success = await makeMove(fromPos, toPos);
                    if (success) {
                        clearSelection();
                    ...
                    if (piece !== ' ') {
                        ...
                        if ((playerColor === 'white' && isWhitePiece) || 
                            ...
                            showNotification('Invalid Selection', 'You can only move your own pieces!', 'warning');
                        }
                    }
                }
            } finally {
        ...
        // Optimized square selection with visual feedback
        function selectSquare(row, col) {
            // Clear previous selection efficiently
            const previousSelected = document.querySelector('.square.selected');
            if (previousSelected) {
            ...
            if (square) {
                ...
                square.style.willChange = 'transform, box-shadow';
                requestAnimationFrame(() => {
                    square.style.transform = 'scale(1.02)';
                    setTimeout(() => {
        ...
        // Enhanced notification system with performance optimizations
        let notificationQueue = [];
        let isShowingNotification = false;
        
        function showNotification(header, content, type) {
            // Queue notifications to prevent overlap
            notificationQueue.push({ header, content, type });
        ...
        function processNotificationQueue() {
            ...
            const { header, content, type } = notificationQueue.shift();
            ...
            requestAnimationFrame(() => {
                ...
                notification.className = 'notification'; // Reset all classes
                if (type === 'white') {
                ...
                // Hide after 3.5 seconds and process next
                setTimeout(() => {
